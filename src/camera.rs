...
impl Camera {
    ...
    pub fn rays(
        ...
        width: usize,
        height: usize,
    ) -> impl ParallelIterator<Item = (usize, usize, Line3d)> {
        ...
        (0..width)
            ...
            .map(move |(x, y)| {
                let pixel_center =
                    canvas_corner + pixel_right * (x as f64 + 0.5) + pixel_down * (y as f64 + 0.5);

                (
                    x,
                    y,
                    Line3d::new(pixel_center, camera_center.unit_to(&pixel_center)),
                )
            })
    }
}
